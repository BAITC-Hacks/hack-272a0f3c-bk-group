...
(() => {
  ...
  function entitiesView(kind){
    ...
    const headers=kind==='products'?['Артикул','Наименование','Единица','Цена']:kind==='counterparties'?['Наименование','Тип','Документы']:['Наименование','Товарных позиций'];
    const body=paginate(rows).map(row=>kind==='products'?`<tr><td>${opsEsc(row.sku||'—')}</td><td>${link(row.name,'open-entity',row.id)}</td><td>${opsEsc(row.unit||'—')}</td><td class="num">${money(row.price)}</td></tr>`:kind==='counterparties'?`<tr><td>${link(row.name,'open-entity',row.id)}</td><td>${opsEsc(typeLabels[row.type]||row.type)}</td><td>${opsFmt(ops.documents.filter(doc=>doc.counterparty_id===row.id).length)}</td></tr>`:`<tr><td>${link(row.name,'open-entity',row.id)}</td><td>${opsFmt((ops.data.stock||[]).filter(stock=>stock.warehouse_id===row.id&&Number(stock.quantity)!==0).length)}</td></tr>`).join('');
    return toolbar({create:'new-entity',label:labels[kind],extra:kind==='products'?'<button data-ops-action="import-report">Из плана закупок</button>':''})+(rows.length?table(headers,body):`<div class="ops-panel">${empty(ops.query?'Ничего не найдено':`${kind==='products'?'Товаров':kind==='counterparties'?'Контрагентов':'Складов'} пока нет`,ops.query?'Измените запрос и повторите поиск.':'Добавьте первую запись. Она будет доступна во всех документах.','new-entity',`Добавить ${labels[kind].toLowerCase()}`)}</div>`)+pagination(rows.length);
  }
  ...
  function journalView(){const rows=ops.journal.filter(row=>searchMatch([row.description,row.action,row.entity_type,row.created_at]));return toolbar({exportable:true})+(rows.length?table(['Дата и время','Действие','Описание'],paginate(rows).map(row=>`<tr><td>${opsEsc(String(row.created_at||'').replace('T',' ').slice(0,19))}</td><td>${opsEsc(({create:'Создание',update:'Изменение',post:'Проведение',cancel:'Отмена',import:'Импорт'})[row.action]||row.action)}</td><td>${opsEsc(row.description||row.entity_type)}</td></tr>`).join('')):`<div class="ops-panel">${empty('Журнал пока пуст','Здесь появятся сохранения, проведения, отмены и импорт товаров.')}</div>`)+pagination(rows.length);}
  ...
  function exportCurrent(){
    ...
    else if(ops.tab==='products'){headers=['Артикул','Наименование','Единица','Цена'];rows=ops.data.products.filter(row=>searchMatch([row.name,row.sku])).map(row=>[row.sku,row.name,row.unit,row.price]);}
    ...
    else if(ops.tab==='journal'){headers=['Дата','Действие','Тип','Описание'];rows=ops.journal.filter(row=>searchMatch([row.description,row.action,row.entity_type,row.created_at])).map(row=>[row.created_at,row.action,row.entity_type,row.description]);}
